...
async function refresh(){try{const r=await fetch('/status.json',{cache:'no-store'});lastStatus=await r.json();apply(lastStatus);}catch(e){[energy,energyTarget,passive,stop,clear,rwlog,zero,target,shadowTarget].forEach(x=>lock(x,true));}}setInterval(refresh,500);refresh();
</script></body></html>
)HTML";
