...
void setup() {
  ...
  delay(300);
  Serial.println();
  Serial.println("AtomS3R V46e MEKF motor-driven dynamic validation");

  auto cfg = M5.config();
  cfg.serial_baudrate = 0;
  cfg.internal_imu = true;
  M5.begin(cfg);
  Serial.printf("V46e identity: board=%d imu_type=%d M5Unified=%s M5GFX=%s AHRS=%s base=%s attitude=%s\n",
  ...
  displayLine("V46e MEKF", "V7 MOTOR VALIDATION");
